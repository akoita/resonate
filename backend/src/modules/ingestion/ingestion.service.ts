...
export class IngestionService {
  ...
  async handleFileUpload(input: {
    ...
    let sourceTrackDisclosure: {
      level: "NONE" | "PARTLY" | "ALL";
      facets: string[];
      source: AiDisclosureSource;
    } | null = null;

    // If catalogTrackId is provided, fetch the audio from catalog and preserve
    // its persisted disclosure. Reusing catalog audio is not itself evidence
    // that the audio was generated by Resonate.
    if (input.catalogTrackId && input.files.length === 0) {
      const sourceTrack = await prisma.track.findUnique({
        where: { id: input.catalogTrackId },
        select: {
      ...
      if (sourceTrack && sourceTrack.aiDisclosureLevel !== "UNDECLARED") {
        sourceTrackDisclosure = {
          level: sourceTrack.aiDisclosureLevel,
          facets: sourceTrack.aiContributionFacets,
          source: normalizeAiDisclosureSource(sourceTrack.aiDisclosureSource),
        };
      }
    ...
    let disclosures: Array<{
      level: "NONE" | "PARTLY" | "ALL";
      facets: string[];
      source: AiDisclosureSource;
    }>;
    try {
      disclosures = input.files.map((_, index) => {
        if (sourceTrackDisclosure) {
          return sourceTrackDisclosure;
        }
        const normalized = normalizeAiDisclosureInput(
          input.metadata?.tracks?.[index]?.aiDisclosure,
        );
        return {
          level: normalized.level as "NONE" | "PARTLY" | "ALL",
