...
describe('IngestionController (e2e)', () => {
  ...
  it('POST /ingestion/cancel/:releaseId passes JWT userId to service', async () => {
    await request(app.getHttpServer())
      .post('/ingestion/cancel/rel-1')
      .set('Authorization', `Bearer ${token}`)
      .expect(201);

    expect(mockIngestionService.cancelProcessing).toHaveBeenCalledWith('rel-1', 'user-1');
  });
