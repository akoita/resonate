...
| `IMAGE_OPTIMIZER_MINIMUM_CACHE_TTL` | Frontend build-time | Optional Next image optimizer cache TTL in seconds. Defaults to `0` and clamps to `0..86400`. Keep the default `0` unless release-artwork invalidation/versioning is addressed: nonzero values can keep mutable release artwork stale until expiry because public release URLs are currently stable. |
