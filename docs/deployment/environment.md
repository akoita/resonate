...
| `ANALYTICS_WAREHOUSE_TARGET` | Backend | Warehouse loader target provider. Defaults to `local_json`, which writes idempotent JSONL layer files outside process memory. Set `bigquery_insert_all` for legacy streaming inserts or opt into `bigquery_batch` for bounded transactional loads through Google ADC. Batch needs explicit from/to dates, serialized writers, and drained streaming buffers; see [batch operations](../operations/analytics-batch.md). |
...
| `ANALYTICS_ACTOR_ID_SALT` | Backend | **Required outside local/test development.** Stable secret used to derive pseudonymous analytics actor IDs. Startup and identity derivation fail closed when it is absent from a deployed process. Treat it as non-rotatable; changing it requires a planned analytics identity migration or exports and erasures can no longer find historical events. Every component that derives or resolves analytics identities must receive the same value. See #1796. |
| `ANALYTICS_ACTOR_ID_SALT_ALLOW_INSECURE_FALLBACK` | Backend | Emergency compatibility exception only. When exactly `true`, permits a deployed process without `ANALYTICS_ACTOR_ID_SALT` to fall back to `JWT_SECRET` or the local constant and emits a startup warning. This restores the unsafe legacy behavior and must be temporary, deliberate, and monitored. Leave unset/false normally. |
| `GOVERNANCE_VALIDATION_ENABLED` | Backend (non-production only) | Opt-in for the governance validation harness (`dist/scripts/governance_validation.js`), which **seeds fake data**. Must be exactly `true` for the harness to run; it additionally refuses when any environment label indicates production. Never set this in production. See `docs/operations/governance_validation_harness.md`. |
| `GOVERNANCE_VALIDATION_RUN_ID` | Backend (non-production only) | Identifier tying a harness seed phase to its later verify and cleanup phases, which run as separate executions. Matches `[a-z0-9][a-z0-9-]{0,23}`, defaults to `default`. Deliberately not clock-derived, because verify must rediscover the rows seed created. |
...
| `RESONATE_ENVIRONMENT_ID` | Backend | Stable identity of this backend environment, returned by `GET /health` (#1199) and used to label analytics events (#1806). Defaults to `local` for health/session-reset behavior. Set a distinct value per environment (e.g. `staging`, `prod`, or a name containing that tier) — when it changes, browsers pointed at it prompt a guided client session reset, which is what makes a backend migration (#915) graceful for users. Analytics recognizes `local`, `dev`, `staging`, and `prod` (including those tier tokens inside a longer identity); an unknown or missing identity is labelled `dev`, never inferred as `prod` from `NODE_ENV`. |
...
| `PROMPT_MODERATION_ENABLED` | Backend | Prompt-safety moderation for the self-hosted generation path (#1343). **Default `true` (on)** — the self-hosted Stable Audio worker has no vendor filter, so `PromptModerationService` screens generation prompts for PUP categories 2–4 (illegal/malicious incl. safety-bypass, deceptive voice-impersonation, sexually explicit, and zero-tolerance minor-sexualization) before dispatch; violations return HTTP 422 `prompt_rejected` and emit `remix.policy_rejected`. Set to `false` only to disable (not recommended in production) |
| `REMIX_GENERATION_PROVIDER_KIND` | Backend | Selects the remix generation implementation: `stub` (default), `lyria` (#1162 — reuses the catalog Lyria stack: `GOOGLE_AI_API_KEY` or `LYRIA_PROJECT_ID`/`LYRIA_LOCATION`, plus the storage provider for draft output), or `audio-conditioned` (#1182 slice 4 — sends the mixed unmuted stems to the self-hosted Stable Audio 3 worker for a true audio-conditioned draft). `REMIX_GENERATION_ENABLED` remains the master gate regardless of kind |
| `REMIX_AUDIO_WORKER_URL` | Backend | Base URL of the Stable Audio 3 worker for `audio-conditioned` generation (`POST /generate`). Defaults to `http://localhost:8000` |
...
| `REMIX_WORKER_PREWARM_TTL_SECONDS` | Backend | Optional debounce window for Remix Studio's fire-and-forget Stable Audio worker prewarm ping. Defaults to `600` seconds; only applies when `REMIX_GENERATION_ENABLED=true` and `REMIX_GENERATION_PROVIDER_KIND=audio-conditioned` |
...
| `GENERATION_PRICE_CENTS_PER_30S` | Backend | Sell price of AI generation in USD cents per 30 seconds for the generation-credit meter (#1334, ADR-BM-3). Default `10` (= $0.10/30s; internal cost baseline is ~6¢, ~40% margin). A generation costs `ceil((durationSeconds / 30) × price)` cents, debited from the user's prepaid credit balance before Lyria/remix drafting. Staging: beyond the starter allowance, credits enter only via the operator `POST /credits/grant` path (or `make grant-credits`); live fiat top-up is deferred |
| `GENERATION_CREDITS_SIGNUP_STARTER_CENTS` | Backend | One-time free generation-credit starter allowance (USD cents) provisioned per account on first use of the meter (balance read or first generation), so new users can try generation before the meter charges (#1334, ADR-BM-3). Default `0` = disabled (production-safe, operator-grant-only). Staging sets `100` (= $1.00 ≈ 5 min of generation). Idempotent per account; a customer-acquisition free tier that subsidises no fan→artist payout (ADR-BM-4 untouched) |
...
| `X402_PERSONAL_PRICE_USD` / `X402_REMIX_LICENSE_USD` / `X402_COMMERCIAL_LICENSE_USD` | Backend | Optional x402 license price overrides. Defaults follow the canonical Layer 3 table in `docs/rfc/business-model.md`: personal `$0.05`, remix `$5`, commercial `$25` |
| `X402_PERSONAL_FEE_BPS` / `X402_REMIX_FEE_BPS` / `X402_COMMERCIAL_FEE_BPS` | Backend | Optional x402 take-rate overrides in basis points. Defaults follow the canonical Layer 3 table: personal `1500`, remix `1000`, commercial `1000`. Facilitator-mode settlements account for this off-chain; contract-settlement mode uses the marketplace contract split |
...
| `PROTOCOL_FEE_BPS` | Contracts / deployment handoff | Marketplace protocol fee basis points promoted from deployment outputs for app/deploy consumers. App UI should prefer live `protocolFeeBps` / quote fields and must not hardcode this value |
| `FEE_RECIPIENT` | Contracts / deployment handoff | Marketplace protocol fee recipient promoted from deployment outputs. Use deployment/secret-manager handoffs for shared environments rather than source defaults |
...
| `COMMUNITY_MODERATION_ASSIST_STRATEGY` | Backend | Optional moderation assist strategy for `/admin/community/moderation`. Defaults to `deterministic`; set `model-assisted` to enable model-backed summaries when `GOOGLE_AI_API_KEY` is present. Missing credentials, timeouts, malformed output, or unsupported values fall back to deterministic assist |
| `COMMUNITY_MODERATION_ASSIST_MODEL` | Backend | Optional model name for model-backed community moderation assist. Falls back to `VERTEX_AI_MODEL`, then the backend default model |
| `COMMUNITY_MODERATION_ASSIST_TIMEOUT_MS` | Backend | Optional timeout for model-backed community moderation assist. Defaults to `6000` and is clamped between `1000` and `30000` |
| `COMMUNITY_MODERATION_ASSIST_MAX_MODEL_REPORTS` | Backend | Optional cap on model-backed assists per moderation queue response. Defaults to `10`, accepts `0` for deterministic queue hydration, and is clamped to `25` |
| `COMMUNITY_MODERATION_ASSIST_CONCURRENCY` | Backend | Optional per-process concurrency cap for model-backed moderation assist calls. Defaults to `3` and is clamped between `1` and `5` |
