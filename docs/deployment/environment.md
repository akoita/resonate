...
| `ANALYTICS_ACTOR_ID_SALT` | Backend | **Required for any deployment that must honour an erasure request**, despite the fallback. Salt for deriving pseudonymous analytics actor IDs. Falls back to `JWT_SECRET`, then a local-dev salt — so an unset salt makes analytics identity depend on an authentication secret, and rotating that secret orphans every actor ID already written, after which exports omit those events and erasures report success without removing them. **Treat as non-rotatable** once set; exclude it from routine secret rotation. Every component that derives or resolves analytics identities must receive the same value. See #1796. |
