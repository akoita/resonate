...
| `GENERATION_PRICE_CENTS_PER_30S` | Backend | Sell price of AI generation in USD cents per 30 seconds for the generation-credit meter (#1334, ADR-BM-3). Default `10` (= $0.10/30s; internal cost baseline is ~6¢, ~40% margin). A generation costs `ceil((durationSeconds / 30) × price)` cents, debited from the user's prepaid credit balance before Lyria/remix drafting. Staging: credits enter only via the operator `POST /credits/grant` path; live fiat top-up is deferred |
