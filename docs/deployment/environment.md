...
| `IMAGE_OPTIMIZER_MINIMUM_CACHE_TTL` | Frontend build-time | Optional Next image optimizer cache TTL in seconds. Defaults to `0` and clamps to `0..86400`. Release covers and Shows campaign visuals use server-owned revision path segments, so a successful replacement produces a new optimizer key without waiting for expiry; legacy unversioned paths remain readable and resolve the current image. Keep `0` as the rollback value. The 2026-09-01 [#1666 staging runbook](../operations/artwork-cache-staging-validation.md) proof passed revision-key replacement and restoration but failed the 100 KiB Home image budget, so staging retains `0` and no production value was selected. A future nonzero deployment still requires comparable same-machine cold/warm evidence and a passing image budget. |
