...
> **Stack chosen:** ZeroDev Kernel v3 (smart accounts) + WebAuthn Passkeys (auth) + ZeroDev hosted bundler/paymaster. Privy was dropped in favour of native passkey authentication via ZeroDev's `@zerodev/passkey-validator`.

See [Account Abstraction Integration](../account-abstraction.md) for the implemented architecture.
