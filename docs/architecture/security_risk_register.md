...
| Payments | Fabricated settlement events skewing payout analytics | Medium | The prototype `POST /payments/initiate`, `/split-config`, `/split` and `/confirm` routes were removed (#1890), so no client request can publish `payment.settled`. Remaining payment request bodies are class DTOs enforced by the global `ValidationPipe` |
