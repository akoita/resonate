...
The artist analytics endpoints now read from normalized analytics layers
generated by `AnalyticsWarehouseExportService`:

- `GET /analytics/artist/:id`
- `GET /analytics/artist/:id/v1`

The service consumes `analytics_facts` for play and payout report totals and
uses fact dimensions for compatibility fields such as track title, session,
source, and payout asset metadata. This keeps current response shapes stable
while moving reports away from raw in-memory event aggregation.
...
- Durable event ledger facts/views for current backend reads; production
  warehouse loading remains follow-up work.
