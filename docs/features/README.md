...
| [Agent Taste Intelligence](agent_taste_intelligence.md) | `partial` | listeners, agents, backend developers, data/ML developers | `AgentSelectorService`, `AgentBigQueryTasteSignalService`, `AGENT_TASTE_SIGNAL_SOURCE=bigquery`, BigQuery `user_track_recommendation_scores`, `workers/analytics-dataflow/run-agent-taste-materialization.sh`, `workers/analytics-dataflow/sql/agent_taste_intelligence_baseline.sql`, [#977](https://github.com/akoita/resonate/issues/977) | Optional BigQuery-backed user-track taste scores now blend into agent recommendation scoring as an additive, explainable signal. The default deterministic selector remains unchanged when the feature is disabled, BigQuery is unavailable, or no score exists for a candidate. A parameterized baseline materialization runner now turns playback, save, skip, replay, purchase, session-intent, and agent-selection events into the serving score table with verification queries for freshness and coverage; scheduled infrastructure, offline ML promotion, analytics-derived explanations, Session Intent feedback, preset instrumentation, and recommendation quality reporting remain roadmap work. |
