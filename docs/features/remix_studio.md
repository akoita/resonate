...
- "Describe it" (#1900, slice S3 of epic #1896). At the top of Create → Mix
  ...
  **Undo** restores the previous state.
  ...
  - **Follow-up.** An opt-in model-backed parser for free-form phrasing is
    #1907.
- Beat maker (#1902, slice S5 of epic #1896). Anyone can add a drum beat
  that locks to the song's tempo, with no samples to license.
  - **Recipe.** `remix-beat/v1` is stored as `RemixProject.beat` (null = no
    beat): a kit (Punchy, 808, Lo-fi), a 1-bar 16-step pattern for kick,
    snare, clap, hat and open hat, swing ("Groove"), a level, per-block
    on/off, and a persisted mute. A muted beat is left out of the render, as
    stem mutes are, so the preview and the render agree.
    - A beat needs a bar grid (measured tempo). Otherwise the studio says so
      plainly.
  - **Synthesized in code.** Every drum is generated by deterministic math:
    - kick: a pitch-dropping sine
    - snare: a sine body plus filtered seeded noise
    - clap: noise bursts
    - hats: high-passed noise

    No audio samples are shipped, so there is nothing to license, and the
    preview and render produce the same sound. A parity fixture
    (`backend/src/modules/remix/remix-beat-v1.parity.json`) holds both
    engines to the same one-shots, hit list and track samples.
  - **Timing.**
    - Bars are anchored at each timeline block, so the beat follows the
      structure (S2).
    - Pickup blocks get no beat.
    - Swing delays the off-beat sixteenths.
    - The beat track follows the Speed control like the stems.
  - **Render.** The beat is rendered to one PCM track over the timeline and
    mixed as one extra input: varispeed → level → master effects and
    loudness. It sends to the reverb at the master Space level. With no beat,
    the render graph is byte-identical.
  - **Controls.**
    - A **Beat** lane appears in the Session: level, mute/solo and per-block
      cells.
    - Create → Mix stems → **Add a beat** has presets (Four on the floor, Boom
      bap, Trap, Breakbeat, Half-time), a kit picker, a 5×16 step grid, a
      Groove slider, and Remove beat.
  - **Provenance.**
    - A synthesized beat is neither AI nor source audio, so grounding stays
      `stem_audio`.
