...
with a safe crop. Explorer cards try each source in turn (declared image, then
the card and hero visual endpoints), including images that failed before the
page became interactive, and only when none loads show a typographic city
poster shared with the Home live-events rail — never a broken image.
...
The repository also ships five media-rich, repeatable sample concepts:
SennaRin in Paris, Felicia Farerre in Dublin, Leona Lewis in Lagos, Aya
Nakamura in Montréal, and Tiken Jah Fakoly in Brooklyn (#1873). Each combines a source-grounded artist biography with
...
| Artist ↔ campaign navigation | implemented | The campaign detail page links to the artist ("Listen to <artist> on Resonate") — the artist profile when the campaign carries an `artistId`, otherwise the catalog credit page — and the "Meet the artist" heading links there too. Public artist profiles and release pages show a banner for up to two of the artist's still-actionable campaigns (`ArtistCampaignLink`, matched by artist profile id, falling back to an exact artist-name match only when either side has no id). |
