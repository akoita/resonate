...
| Moderation console | implemented | `/admin/community/moderation` exposes open community reports, bounded room/message context, membership status counts, and admin actions for dismiss, delete message, remove/ban member, pause room, and archive room. AI summaries and automated abuse scoring remain advisory follow-up work tracked in [#1083](https://github.com/akoita/resonate/issues/1083); portable on-chain (NFT) community membership boundaries are explored in [#1084](https://github.com/akoita/resonate/issues/1084). |
