...
| Holder benefit engine | in-progress | Backend foundation for private badge, role, ownership, campaign-support, and redemption eligibility is implemented from [#998](https://github.com/akoita/resonate/issues/998). [#1126](https://github.com/akoita/resonate/issues/1126) adds artist/operator rule management APIs and an artist Community tab panel for artist holders, campaign supporters, collector badges, and holder roles. [#1128](https://github.com/akoita/resonate/issues/1128) adds the `/community` listener unlocked-benefits panel with private eligibility reads and redemption actions. Public profile benefit showcase badges remain planned. |
...
It adds private badge, role, benefit-rule, and redemption foundations plus
artist/operator rule management. Listener-facing unlocked-benefit discovery and
redemption now live on `/community`; public profile benefit showcase badges are
still deferred. Managed rule and listener benefit responses continue to hide raw
proof details and wallet addresses.
