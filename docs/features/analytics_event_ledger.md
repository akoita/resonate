...
- `playlist.shared` — a listener copied a public playlist link from the owner
  ...
  and rejects unknown channels or a missing or non-id-shaped `playlistId`
  (`[A-Za-z0-9_-]{1,64}`); titles, URLs, and
  other free text are dropped. Before this was allowlisted the endpoint
  answered 400 and the web client's events were lost.
...
| Product funnel instrumentation | Implemented through `POST /analytics/product/event`, `web/src/lib/api.ts`, and `web/src/lib/productAnalytics.ts`; authenticated first-party UI code emits allowlisted pseudonymous events for artist onboarding, artist action cockpit impressions/clicks, wallet connection/faucet moments, agent budget changes, artist upload steps, playlist creation/update/add/remove/play/share, home and marketplace search submission/result clicks, marketplace checkout/purchase intent, and library settings updates. The backend attaches the pseudonymous actor, sanitizes payload fields, rejects unsupported event names, and keeps payloads to scalar/array analytics facts rather than free-form user text. |
